...
public class QueryDslOrderRepository {
    ...
    public List<UserOrder> search(OrderSearchCondition condition){
        ...
        if (condition.getMinPrice() != null) {
            builder.and(
                    userOrder.price.goe(condition.getMinPrice())
            );
        }
        //  최대 금액
        if (condition.getMaxPrice() != null) {
            builder.and(
                    userOrder.price.loe(condition.getMaxPrice())
            );
        }
        //  회원 이름으로 검색
        ...
        //  쿼리를 동적으로 생성해서 리턴
        return queryFactory.selectFrom(userOrder)
                .join(userOrder.user, users)
                .fetchJoin()    //  두 테이블 조인해서 한방에 가져옴
                .where(builder)
                .fetch();
    }
}
